    ...
    def create_ssl_connection(self, hostname, port, timeout, **kwargs):
        ...
        def create_connection(ipaddr, timeout, queobj):
            ...
            try:
                ...
                if not validate:
                    ssl_sock = ssl.wrap_socket(sock, ssl_version=self.ssl_version, ciphers='AES128-SHA', do_handshake_on_connect=False)
                else:
                    ssl_sock = ssl.wrap_socket(sock, ssl_version=self.ssl_version, ciphers='AES128-SHA', cert_reqs=ssl.CERT_REQUIRED, ca_certs=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cacert.pem'), do_handshake_on_connect=False)
